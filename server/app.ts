...
const memberName = (member: RelationshipMember) => member.displayName || member.fullName;
...
const deterministicRelationship = (
...
const maybeAiRelationship = async (fallback: RelationshipResult, fromName: string, toName: string) => {
...
app.post("/api/spaces/:spaceSlug/ai/explain-relationship", ...requireSpaceRead, async (req, res) => {
  try {
    if (!req.familySpace) {
      res.status(500).json({ error: "FamilySpace context not loaded." });
      return;
    ...
    res.json({
      ...result,
      pathMemberIds: result.path.map((item) => item.id),
    });
  } catch (error) {
