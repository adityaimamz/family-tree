...
import { ChevronLeft, ChevronRight, FileJson, Save, Trash2 } from "lucide-react";
...
import { ConfirmDialog, FilterSelect, PrimaryButton, SecondaryButton } from "./ui";
import { FormSection } from "./forms/FormSection";
import { FormStepper, formSteps } from "./forms/FormStepper";
import type { FormStep } from "./forms/FormStepper";
import { RelationshipSelector } from "./forms/RelationshipSelector";
import { MemberPreviewCard } from "./forms/MemberPreviewCard";
    ...
    <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_340px] xl:gap-6">
      ...
      <div className="grid min-w-0 gap-4 xl:gap-6">
        <AnimatePresence mode="wait">
          {step === "personal" && (
            <motion.div key="personal" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ type: "spring", stiffness: 140, damping: 22 }}>
          ...
          {step === "family" && (
            <motion.div key="family" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ type: "spring", stiffness: 140, damping: 22 }}>
          ...
          {step === "notes" && (
            <motion.div key="notes" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ type: "spring", stiffness: 140, damping: 22 }}>
              <FormSection title="Photos and Biography" description="Add warm and readable notes. Photos can be URLs; if empty, initials avatar is created automatically.">
                <div className="md:col-span-2">
                  <PhotoUploadField folder="members" label="Foto" value={member.photo ?? ""} onChange={(value) => update("photo", value || null)} />
                </div>
...
export { FormStepper } from "./forms/FormStepper";
export { RelationshipSelector } from "./forms/RelationshipSelector";
export { MemberPreviewCard } from "./forms/MemberPreviewCard";
