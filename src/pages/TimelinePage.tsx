...
import { TimelineStoryGenerator } from "../components/ai";
...
export const TimelinePage = () => {
  ...
  const { currentSpace, members, timeline, summary, membership, canEdit, addToast } = useSpaceStore();
  ...
  const timelineCount = summary?.timelineCount ?? allEvents.length;
  const generationsCount = summary?.generationsCount ?? new Set(members.map((m) => m.generation)).size;
    ...
    <motion.div {...pageTransition}>
      <PageShell>
        ...
        <section className="mb-7">
          <TimelineStoryGenerator
            spaceSlug={spaceSlug || ""}
            familySpaceName={currentSpace?.name || "Family"}
            role={role}
            panelId="ai-studio-timeline-story"
            summary={summary}
